...
- [4. Инструмент Verilator](#4-инструмент-verilator)
...
- [5. Описание работы виртуального стенда 8 разрядного АЛУ на виртуальной плате DE10-Lite](#5-описание-работы-виртуального-стенда-8-разрядного-алу-на-виртуальной-плате-de10-lite)
  ...
  - [5.10 Таблица сигналов](#510-таблица-сигналов)
- [6. Описание работы виртуальный стенд 16 разрядного АЛУ на виртуальной плате DE2-115](#6-описание-работы-виртуальный-стенд-16-разрядного-алу-на-виртуальной-плате-de2-115)
  ...
  - [6.3 Требования](#63-требования)
    ...
    - [6.3.2 Для Verilog сервера](#632-для-verilog-сервера)
...
![alt text](/Lab_2/Lab_2_theory/Images/screnshots/CLA.png)
...
Рассмотрим выполнение арифметической операции сложения двух четырёхразрядных чисел. Пусть $A = 0101 (5), B = 0011 (3)$. В микросхеме инвертированы входы A и B, а также выход F.Поэтому, $A = 1010 (5), B = 1100 (3)$
На входы микросхемы в симуляторе устанавливаются:

$A_{3} = 1, A_{2} = 0, A_{1} = 1, A_{0} = 0$

$B_{3} = 1, B_{2} = 1, B_{1} = 0, B_{0} = 0$
 
Для операции сложения с учётом переноса выбирается код $S_{3} – S_{0} = 1001$ (см. Таблицу команд).
...
$F_{3} – F_{0} = 0110, учитывая инвертированность F =1001 (9)$, 
...
$A = 00010001$ (17), $B = 00110101$ (53), входной перенос $C_0 = 1$.

Разделим операнды на младшие и старшие тетрады и подадим в симулятор:

Для первой (младшей) 74181:

$A_3 = 0$, $A_2 = 0$, $A_1 = 0$, $A_0 = 1$ → $A_{\text{low}} = 0001$

$B_3 = 0$, $B_2 = 1$, $B_1 = 0$, $B_0 = 1$ → $B_{\text{low}} = 0101$

Для второй (старшей) 74181:

$A_7 = 0$, $A_6 = 0$, $A_5 = 0$, $A_4 = 1$ → $A_{\text{high}} = 0001$

$B_7 = 0$, $B_6 = 0$, $B_5 = 1$, $B_4 = 1$ → $B_{\text{high}} = 0011$

Управляющие сигналы:

$S_3$–$S_0 = 1001$ (код сложения)  
$M = 0$ (арифметическая операция)  
$C_0 = 0$ (начальный перенос). В данной микросхеме инвертирован вход C, то есть по сути он будет равен $C_0 = 1$. 
...
- Первая 74181: $F = 0001 + 0101 + 1 = 0111$ → $F_3$–$F_0 = 0111$
- Вторая 74181 с учётом переноса: $F = 0001 + 0011 = 0100$ → $F_7$–$F_4 = 0100$

Итоговый результат:

$F_7$–$F_0 = 01000111$ (в двоичной форме) = 71 (в десятичной форме)

![alt](/Lab_2/Lab_2_theory/Images/screnshots/example-74182.png) 
...
## 6. Описание работы виртуальный стенд 16 разрядного АЛУ на виртуальной плате DE2-115
