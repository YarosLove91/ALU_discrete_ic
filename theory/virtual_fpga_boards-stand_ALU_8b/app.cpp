...
void handle_client(int client_socket, Vde10_lite* top) {
    ...
    if (!root.isMember("switches") || !root["switches"].isArray() || root["switches"].size() != 10) {
        std::cerr << "Invalid or missing 'switches' array." << std::endl;
        close(client_socket);
        return;
    }
    ...
    if (root.isMember("keys") && root["keys"].isArray() && root["keys"].size() >= 2) {
        Json::Value keys = root["keys"];
        top->KEY = 0; 
        if (keys[0].asBool()) top->KEY |= (1 << 0);  
        if (keys[1].asBool()) top->KEY |= (1 << 1);  
    }
    ...
    Json::Value response;
    response["leds"] = Json::Value(Json::arrayValue);
    response["displays"] = Json::Value(Json::arrayValue);

    for (int i = 9; i >= 0; i--) {
        response["leds"].append((top->LEDR >> i) & 1);
    }
