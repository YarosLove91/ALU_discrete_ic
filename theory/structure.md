...
2. Разработка базового тестового стенда Verilator
   ...
   ```cpp
    ...
    Вызов ```dut->eval();``` вычисляет все сигналы в модуле АЛУ, а ```m_trace->dump(sim_time);``` записывает все значения отслеживаемых сигналов в файл дампа формы волны. Как видите, в настоящее время модель вычисляется только по фронтам тактового сигнала.
