...
# Модуль biriscv_frontend: фронтенд процессора biRISC-V

## Функциональное назначение
**Конвейер выборки и предварительной обработки инструкций**, отвечающий за:
- Управление потоком команд (fetch)
- Предсказание ветвлений
- Предварительное декодирование
- Формирование двухпоточного конвейера для dual-issue
...
|---------------------------|--------------|-----------------------------------------------|
| `SUPPORT_BRANCH_PREDICTION` | 1           | Включение предсказания ветвлений              |
| `NUM_BTB_ENTRIES`         | 32           | Размер Branch Target Buffer                   |
| `NUM_BHT_ENTRIES`         | 512          | Размер Branch History Table                   |
| `RAS_ENABLE`              | 1            | Включение стека возвратов (Return Address Stack) |
...
| `rst_i`                   | 1           | Сигнал сброса                                 |
| **Интерфейс кэша**        |             |                                               |
| `icache_inst_i`           | 64          | Считанные инструкции (2x32-битные)            |
| `icache_valid_i`          | 1           | Валидность данных кэша                        |
| **Контроль потока**       |             |                                               |
| `branch_request_i`        | 1           | Запрос на обработку перехода                  |
| `branch_pc_i`             | 32          | Адрес перехода                                |

### 3. Выходные сигналы
| Сигнал                    | Размерность | Описание                                      |
|---------------------------|-------------|-----------------------------------------------|
| **Управление кэшем**      |             |                                               |
| `icache_rd_o`             | 1           | Сигнал чтения кэша инструкций                 |
| `icache_pc_o`             | 32          | Адрес запрашиваемой инструкции                |
| **Конвейерные данные**    |             |                                               |
| `fetch0_instr_o`          | 32          | Инструкция для потока 0                       |
| `fetch1_instr_o`          | 32          | Инструкция для потока 1                       |
| `fetch0_instr_exec_o`     | 1           | Флаг исполнительной инструкции (поток 0)      |

### 4. Внутренние сигналы
| Сигнал                    | Размерность | Описание                                      |
|---------------------------|-------------|-----------------------------------------------|
| `fetch_valid_w`           | 1           | Валидность данных выборки                     |
| `next_pc_f_w`             | 32          | Следующий PC (рассчитанный NPC)               |
| `fetch_pred_branch_w`     | 2           | Предсказание направления ветвления            |

## Структурная схема
```mermaid
graph TB
    FETCH --> NPC[Branch Predictor]
    FETCH --> DECODE
    NPC --> FETCH
    DECODE --> ISSUE[Два конвейерных потока]